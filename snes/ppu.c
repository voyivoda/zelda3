...
static const uint8 kSpriteSizes[8][2] = {{8, 16},  {8, 32},  {8, 64},
                                         {16, 32}, {16, 64}, {32, 64},
                                         {16, 32}, {16, 32}};

static void ppu_handlePixel(Ppu *ppu, int x, int y);
static int ppu_getPixel(Ppu *ppu, int x, int y, bool sub, int *r, int *g,
                        int *b);
static int ppu_getPixelForBgLayer(Ppu *ppu, int x, int y, int layer,
                                  bool priority);
...
#define IS_SCREEN_ENABLED(ppu, sub, layer)                                     \
  (ppu->screenEnabled[sub] & (1 << layer))
#define IS_SCREEN_WINDOWED(ppu, sub, layer)                                    \
  (ppu->screenWindowed[sub] & (1 << layer))
...
void PpuBeginDrawing(Ppu *ppu, uint8_t *pixels, size_t pitch,
  ...
  if (ppu->brightness != ppu->lastBrightnessMult) {
    ...
    for (int i = 0; i < 32; i++)
      ppu->brightnessMultHalf[i * 2] = ppu->brightnessMultHalf[i * 2 + 1] =
          ppu->brightnessMult[i] = ((i << 3) | (i >> 2)) * ppu_brightness / 15;
...
static void PpuWindows_Calc(PpuWindows *win, Ppu *ppu, uint layer) {
  ...
  int t;
  bool w1_ena =
      (winflags & kWindow1Enabled) && ppu->window1left <= ppu->window1right;
  if (w1_ena) {
  ...
  bool w2_ena =
      (winflags & kWindow2Enabled) && ppu->window2left <= ppu->window2right;
  if (w2_ena) {
  ...
  if ((winflags & (kWindow1Enabled | kWindow1Inversed)) ==
      (kWindow1Enabled | kWindow1Inversed))
    w1_bits = ~w1_bits;
  if (w2_ena) {
  ...
  if ((winflags & (kWindow2Enabled | kWindow2Inversed)) ==
      (kWindow2Enabled | kWindow2Inversed))
    w2_bits = ~w2_bits;
...
#define DO_PIXEL(i)                                                            \
  do {                                                                         \
    pixel = (bits >> i) & 1 | (bits >> (7 + i)) & 2 | (bits >> (14 + i)) & 4 | \
            (bits >> (21 + i)) & 8;                                            \
    if ((bits & (0x01010101 << i)) && z > dstz[i])                             \
      dstz[i] = z + pixel;                                                     \
  } while (0)
#define DO_PIXEL_HFLIP(i)                                                      \
  do {                                                                         \
    pixel = (bits >> (7 - i)) & 1 | (bits >> (14 - i)) & 2 |                   \
            (bits >> (21 - i)) & 4 | (bits >> (28 - i)) & 8;                   \
    if ((bits & (0x80808080 >> i)) && z > dstz[i])                             \
      dstz[i] = z + pixel;                                                     \
  } while (0)
#define READ_BITS(ta, tile)                                                    \
  (addr = &ppu->vram[((ta) + (tile) * 16) & 0x7fff], addr[0] | addr[8] << 16)
  ...
  for (size_t windex = 0; windex < win.nr; windex++) {
    ...
    PpuZbufType *dstz =
        ppu->bgBuffers[sub].data + win.edges[windex] + kPpuExtraLeftRight;
...
#define NEXT_TP()                                                              \
  if (tp != tp_last)                                                           \
    ...
    tp = tp_next, tp_next = tp_last - 31, tp_last = tp + 31;
...
static void PpuDrawBackground_2bpp(Ppu *ppu, uint y, bool sub, uint layer,
                                   PpuZbufType zhi, PpuZbufType zlo) {
#define DO_PIXEL(i)                                                            \
  do {                                                                         \
...
#define READ_BITS(ta, tile)                                                    \
  (addr = &ppu->vram[(ta) + (tile) * 8 & 0x7fff], addr[0])
...
// Draw a whole line of a 4bpp background layer into bgBuffers, with mosaic
// applied
static void PpuDrawBackground_4bpp_mosaic(Ppu *ppu, uint y, bool sub,
                                          uint layer, PpuZbufType zhi,
                                          PpuZbufType zlo) {
#define GET_PIXEL()                                                            \
  pixel = (bits) & 1 | (bits >> 7) & 2 | (bits >> 14) & 4 | (bits >> 21) & 8
#define GET_PIXEL_HFLIP()                                                      \
  pixel =                                                                      \
...
#define READ_BITS(ta, tile)                                                    \
  ...
  for (size_t windex = 0; windex < win.nr; windex++) {
    ...
    PpuZbufType *dstz_end =
        ppu->bgBuffers[sub].data + win.edges[windex + 1] + kPpuExtraLeftRight;
...
// Draw a whole line of a 2bpp background layer into bgBuffers, with mosaic
// applied
static void PpuDrawBackground_2bpp_mosaic(Ppu *ppu, int y, bool sub, uint layer,
                                          PpuZbufType zhi, PpuZbufType zlo) {
...
// level6 should be set if it's from palette 0xc0 which means color math is not
// applied
#define SPRITE_PRIO_TO_PRIO(prio, level6)                                      \
  (((prio) * 4 + 2) * 16 + 4 + (level6 ? 2 : 0))
    ...
    if (mosaic_enabled) {
      ...
      do {
        ...
        if ((uint32)(xpos | ypos) > outside_value) {
          ...
          tile =
              ppu->vram[(ypos >> 11 & 0x7f) * 128 + (xpos >> 11 & 0x7f)] & 0xff;
        }
        uint8 pixel =
            ppu->vram[tile * 64 + (ypos >> 8 & 7) * 8 + (xpos >> 8 & 7)] >> 8;
        if (pixel) {
          int i = 0;
          do
            dstz[i] = pixel + z;
      ...
      } while (xpos += dx * w, ypos += dy * w, dstz += w, w = ppu->mosaicSize,
               dstz_end - dstz != 0);
    } else {
      do {
        if ((uint32)(xpos | ypos) > outside_value) {
...
static FORCEINLINE float FloatInterpolate(float x, float xmin, float xmax,
...
static void PpuDrawMode7Upsampled(Ppu *ppu, uint y) {
  // expand 13-bit values to signed values
  uint32 xCenter = ((int16_t)(ppu->m7matrix[4] << 3)) >> 3,
         yCenter = ((int16_t)(ppu->m7matrix[5] << 3)) >> 3;
  ...
  if (*(uint32 *)&ppu->mode7PerspectiveLow == 0) {
    ...
    static const float kInterpolateOffsets[4] = {-1, -1 + 0.25f, -1 + 0.5f,
                                                 -1 + 0.75f};
    for (int i = 0; i < 4; i++)
      m0v[i] = 4096.0f / FloatInterpolate((int)y + kInterpolateOffsets[i], 0,
                                          223, ppu->mode7PerspectiveLow,
                                          ppu->mode7PerspectiveHigh);
  }
  size_t pitch = ppu->renderPitch;
  uint8 *render_buffer_ptr = &ppu->renderBuffer[(y - 1) * 4 * pitch];
  uint8 *dst_start =
      render_buffer_ptr + (ppu->extraLeftRight - ppu->extraLeftCur) * 16;
...
#define DRAW_PIXEL(mode)                                                       \
  ...
  *(uint32 *)dst = (mode ? (ppu->colorMapRgb[pixel] & 0xfefefe) >> 1           \
...
#undef DRAW_PIXEL
  ...
  if (ppu->lineHasSprites) {
    uint8 *dst = dst_start;
    PpuZbufType *pixels =
        ppu->objBuffer.data + (kPpuExtraLeftRight - ppu->extraLeftCur);
    for (size_t i = 0; i < draw_width; i++, dst += 16) {
      uint32 pixel = pixels[i] & 0xff;
      if (pixel) {
        uint32 color = ppu->colorMapRgb[pixel];
        ((uint32 *)dst)[3] = ((uint32 *)dst)[2] = ((uint32 *)dst)[1] =
            ((uint32 *)dst)[0] = color;
        ((uint32 *)(dst + pitch * 1))[3] = ((uint32 *)(dst + pitch * 1))[2] =
            ((uint32 *)(dst + pitch * 1))[1] =
                ((uint32 *)(dst + pitch * 1))[0] = color;
        ((uint32 *)(dst + pitch * 2))[3] = ((uint32 *)(dst + pitch * 2))[2] =
            ((uint32 *)(dst + pitch * 2))[1] =
                ((uint32 *)(dst + pitch * 2))[0] = color;
        ((uint32 *)(dst + pitch * 3))[3] = ((uint32 *)(dst + pitch * 3))[2] =
            ((uint32 *)(dst + pitch * 3))[1] =
                ((uint32 *)(dst + pitch * 3))[0] = color;
      }
    }
  }
...
static NOINLINE void PpuDrawWholeLine(Ppu *ppu, uint y) {
  ...
  // The 6:th bit is automatically zero, math is never applied to the first half
  // of the sprites.
  ...
  uint32 cw_clip_math = ((cwin.bits & kCwBitsMod[ppu->clipMode]) ^
                         kCwBitsMod[ppu->clipMode + 4]) |
                        ((cwin.bits & kCwBitsMod[ppu->preventMathMode]) ^
                         kCwBitsMod[ppu->preventMathMode + 4])
                            << 8;
  ...
  do {
    uint32 left = cwin.edges[windex] + kPpuExtraLeftRight,
           right = cwin.edges[windex + 1] + kPpuExtraLeftRight;
    ...
    uint32 fixed_color =
        ppu->fixedColorR | ppu->fixedColorG << 5 | ppu->fixedColorB << 10;
    if (math_enabled_cur == 0 ||
        fixed_color == 0 && !ppu->halfColor && !rendered_subscreen) {
      // Math is disabled (or has no effect), so can avoid the per-pixel maths
      // check
      uint32 i = left;
      do {
      ...
      uint8 *half_color_map =
          ppu->halfColor ? ppu->brightnessMultHalf : ppu->brightnessMult;
      // Store this in locals
      math_enabled_cur |= ppu->addSubscreen << 8 | ppu->subtractColor << 9;
      // Need to check for each pixel whether to use math or not based on the
      // main screen layer.
      uint32 i = left;
      do {
        ...
        if (math_enabled_cur & (1 << main_layer)) {
          ...
          uint32 r2 = (color2 & 0x1f), g2 = ((color2 >> 5) & 0x1f),
                 b2 = ((color2 >> 10) & 0x1f);
  ...
  if (ppu->extraLeftRight - ppu->extraLeftCur != 0)
    memset(dst_org, 0,
           sizeof(uint32) * (ppu->extraLeftRight - ppu->extraLeftCur));
  if (ppu->extraLeftRight - ppu->extraRightCur != 0)
    memset(dst_org + (256 + ppu->extraLeftRight * 2 -
                      (ppu->extraLeftRight - ppu->extraRightCur)),
           0, sizeof(uint32) * (ppu->extraLeftRight - ppu->extraRightCur));
}
...
static void ppu_handlePixel(Ppu *ppu, int x, int y) {
  ...
  if (!ppu->forcedBlank) {
    ...
    if (ppu->clipMode == 3 || (ppu->clipMode == 2 && colorWindowState) ||
    ...
    if ((mathEnabled && ppu->addSubscreen) || ppu->mode == 5 ||
        ppu->mode == 6) {
  ...
  int row = y - 1;
  uint8 *pixelBuffer =
      (uint8 *)&ppu->renderBuffer[row * ppu->renderPitch +
  ...
  for (int i = 0; i < layerCountPerMode[actMode]; i++) {
    ...
    if (!sub) {
      layerActive = IS_SCREEN_ENABLED(ppu, 0, curLayer) &&
                    (!IS_SCREEN_WINDOWED(ppu, 0, curLayer) ||
                     !ppu_getWindowState(ppu, curLayer, x));
    } else {
      layerActive = IS_SCREEN_ENABLED(ppu, 1, curLayer) &&
                    (!IS_SCREEN_WINDOWED(ppu, 1, curLayer) ||
                     !ppu_getWindowState(ppu, curLayer, x));
    }
    if (layerActive) {
      if (curLayer < 4) {
        ...
        if (ppu->mode == 7) {
          ...
          pixel = ppu_getPixelForBgLayer(ppu, lx & 0x3ff, ly & 0x3ff, curLayer,
                                         curPriority);
        }
      } else {
        // get a pixel from the sprite buffer
        pixel = 0;
        if ((ppu->objBuffer.data[x + kPpuExtraLeftRight] >> 12) ==
            SPRITE_PRIO_TO_PRIO_HI(curPriority))
...
static int ppu_getPixelForBgLayer(Ppu *ppu, int x, int y, int layer,
  ...
  uint16_t tilemapAdr = layerp->tilemapAdr + (((y >> tileBitsY) & 0x1f) << 5 |
                                              ((x >> tileBitsX) & 0x1f));
  ...
  int paletteSize = 4;
  uint16_t plane1 =
      ppu->vram[(layerp->tileAdr + ((tileNum & 0x3ff) * 4 * bitDepth) + row) &
                0x7fff];
  ...
  if (bitDepth > 2) {
    paletteSize = 16;
    uint16_t plane2 = ppu->vram[(layerp->tileAdr +
  ...
  if (bitDepth > 4) {
    paletteSize = 256;
    uint16_t plane3 =
        ppu->vram[(layerp->tileAdr + ((tileNum & 0x3ff) * 4 * bitDepth) + 16 +
                   row) &
                  0x7fff];
    pixel |= ((plane3 >> col) & 1) << 4;
    pixel |= ((plane3 >> (8 + col)) & 1) << 5;
    uint16_t plane4 =
        ppu->vram[(layerp->tileAdr + ((tileNum & 0x3ff) * 4 * bitDepth) + 24 +
                   row) &
                  0x7fff];
...
static void ppu_calculateMode7Starts(Ppu *ppu, int y) {
  ...
  ppu->m7startX =
      (((ppu->m7matrix[0] * clippedH) & ~63) + ((ppu->m7matrix[1] * ry) & ~63) +
       ((ppu->m7matrix[1] * clippedV) & ~63) + (xCenter << 8));
  ppu->m7startY =
      (((ppu->m7matrix[2] * clippedH) & ~63) + ((ppu->m7matrix[3] * ry) & ~63) +
       ((ppu->m7matrix[3] * clippedV) & ~63) + (yCenter << 8));
}
...
static int ppu_getPixelForMode7(Ppu *ppu, int x, int layer, bool priority) {
  ...
  if (!ppu->m7largeField)
    outsideMap = false;
  uint8_t tile =
      outsideMap ? 0 : ppu->vram[(yPos >> 3) * 128 + (xPos >> 3)] & 0xff;
  uint8_t pixel = outsideMap && !ppu->m7charFill
                      ? 0
...
static bool ppu_evaluateSprites(Ppu *ppu, int line) {
  ...
  uint8 spriteSizes[2] = {kSpriteSizes[ppu->objSize][0],
                          kSpriteSizes[ppu->objSize][1]};
  ...
  do {
    ...
    for (int col = 0; col < spriteSize; col += 8) {
      if (col + x > -8 - extra_left_right && col + x < 256 + extra_left_right) {
        ...
        // figure out which tile this uses, looping within 16x16 pages, and get
        // it's data
        int usedCol = oam1 & 0x4000 ? spriteSize - 1 - col : col;
        int usedTile = ((((oam1 & 0xff) >> 4) + (row >> 3)) << 4) |
                       (((oam1 & 0xf) + (usedCol >> 3)) & 0xf);
        uint16 *addr =
            &ppu->vram[(objAdr + usedTile * 16 + (row & 0x7)) & 0x7fff];
        ...
        PpuZbufType *dst =
            ppu->objBuffer.data + col + x + px_left + kPpuExtraLeftRight;

        for (int px = px_left; px < px_right; px++, dst++) {
          int shift = oam1 & 0x4000 ? px : 7 - px;
          uint32 bits = plane >> shift;
          int pixel = (bits >> 0) & 1 | (bits >> 7) & 2 | (bits >> 14) & 4 |
                      (bits >> 21) & 8;
          // draw it in the buffer if there is a pixel here, and the buffer
...
void ppu_write(Ppu *ppu, uint8_t adr, uint8_t val) {
  ...
  case 0x13: {
    ppu->bgLayer[(adr - 0xd) / 2].hScroll =
        ((val << 8) | (ppu->scrollPrev & 0xf8) | (ppu->scrollPrev2 & 0x7)) &
        0x3ff;
    ppu->scrollPrev = val;
    ppu->scrollPrev2 = val;
    break;
  ...
  case 0x12:
  case 0x14: {
    ppu->bgLayer[(adr - 0xe) / 2].vScroll =
        ((val << 8) | ppu->scrollPrev) & 0x3ff;
    ppu->scrollPrev = val;
    break;
  }
  case 0x15: { // VMAIN
  ...
  case 0x19: { // VMDATAH
    ...
    ppu->vram[vramAdr & 0x7fff] =
        (ppu->vram[vramAdr & 0x7fff] & 0x00ff) | (val << 8);
    if (ppu->vramIncrementOnHigh)
      ppu->vramPointer += ppu->vramIncrement;
    break;
  }
  ...
  case 0x32: { // COLDATA
    if (val & 0x80)
      ppu->fixedColorB = val & 0x1f;
    if (val & 0x40)
      ppu->fixedColorG = val & 0x1f;
    if (val & 0x20)
      ppu->fixedColorR = val & 0x1f;
    break;
  }
  case 0x33: {
    assert(val == 0);
